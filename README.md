...
| `SidebarLayout` | Two-pane layout with a pointer- and keyboard-resizable sidebar (`storageKey` persists the width). `activePane` can show either pane at full width. Pair with a `flushBody` window so the sidebar runs edge-to-edge. |
| `MasterDetailLayout` | Responsive list/detail composition with container-width compact navigation, Back focus restoration, and preserved mounted panes. [Usage and keyboard help](docs/master-detail-layout.md). |
