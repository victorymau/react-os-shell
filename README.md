...
| `MilestoneTimeline` | The static sibling: one dot per milestone on a date axis, no scrubber. Labels are packed into up to four lanes — two above the bar, two below — by their **measured** width, so four DFM revisions inside a fortnight stack instead of overprinting; a label that fits no lane collapses and reveals on hover or keyboard focus. A milestone with **no date gets no position**, because the only position available would be a date the record does not have: it is listed as pending beside the bar (three lines, then `+N more`) and takes no part in the range, the fill or a phase bracket. `phase` groups of 2+ get a bracket marking parallel work, and the fill never runs past today. `packLabelLanes` is the pure packer, exported from the module for its own spec. |
