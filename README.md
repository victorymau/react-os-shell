...
| `MilestoneTimeline` | The static sibling: one dot per milestone on a date axis, no scrubber. The axis is **piecewise linear**: every stretch between two dates gets at least 48 px, and the axis compresses idle stretches longer than 30% of the track — a break glyph marks them, with a `title` saying how many days it hides. Labels are packed into up to four lanes — two above the bar, two below — by their **measured** width, so four DFM revisions inside a fortnight stack instead of overprinting; lanes go to the milestones that settle something before the revisions that led up to them, and a label that fits no lane collapses and reveals on hover or keyboard focus. A milestone with **no date gets no position**, because the only position available would be a date the record does not have: it is listed as pending beside the bar (three lines, then `+N more`) and takes no part in the range, the fill or a phase bracket. `phase` groups of 2+ get a bracket marking parallel work, and the fill never runs past today. `compressTimeAxis` and `packLabelLanes` are the pure helpers, exported from the module for their own specs. |
