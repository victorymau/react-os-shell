...
import { bundledApps, utilityApps, gameApps, mailApps, documentApps, webApps } from 'react-os-shell/apps';
...
const TOP_LEVEL_ROUTES = new Set(['/spreadsheet', '/notepad', '/todo', '/documents', '/email', '/calendar', '/preview', '/files', '/browser']);
const lookupLabel = (to: string) =>
  (utilityApps as any)[to]?.label
  ?? (gameApps as any)[to]?.label
  ?? (mailApps as any)[to]?.label
