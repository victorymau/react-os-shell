...
- **Exposé: hover highlight + animated exit** — hovered thumbnails get a blue ring and lift above their neighbours, with the title underneath shown on a blue selection pill (mirrors macOS Mission Control). Clicking a thumbnail no longer snaps back instantly — every window glides back to its real position over ~280 ms while the picked one is brought to the front, and the dim backdrop fades out over the same window so the whole transition reads as a single coordinated motion.
